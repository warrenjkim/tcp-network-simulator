...
Heap *heap_shrink(Heap *heap) {
    struct packet *temp = (struct packet *)(malloc(WINDOW_SIZE * sizeof(struct packet)));
...
Heap *heap_grow(Heap *heap) {
    struct packet *temp = (struct packet *)(malloc((size_t)(1.5 * heap->capacity * sizeof(struct packet))));
...
size_t queue_pop(Queue *queue, unsigned short seqnum) {
    size_t popped = 0;
    Node *front = queue->front;
    while (front && 0 < queue->size && front->data[front->head].seqnum < seqnum) {
        front->head++;
        queue->size--;
        popped++;
